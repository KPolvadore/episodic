...
const ParallaxScrollView = forwardRef<Animated.ScrollView, Props>(
  ({ children, headerImage, headerBackgroundColor }, forwardedRef) => {
    const backgroundColor = useThemeColor({}, "background");
    const colorScheme = useColorScheme() ?? "light";
    const innerRef = useAnimatedRef<Animated.ScrollView>();
    const scrollOffset = useScrollOffset(innerRef);

    useImperativeHandle(forwardedRef, () => innerRef.current as any, [
      innerRef,
    ]);
